<?php
    ...
    if (isset($_GET['id'])) $edytuj_id = $_GET['id'];
        ...
        $_SESSION[blad] = "Nie można edytować działu. Spróbuj ponownie póżniej";
        header("Location: firma.php");
        exit();
    }
    ...
    if (isset($_POST['nazwa'])){
        $zap = "UPDATE dzialy SET nazwa = '".$_POST['nazwa']."' WHERE ID = ".$edytuj_id;
        if (mysqli_query($pol, $zap)){
            $_SESSION['sukces'] = "Zaktualizowano dział";
            header("Location: firma.php");
            exit();
        }
        else{
            $_SESSION['blad'] = "Nie udało się zaktualizować działu. Spróbuj ponownie póżniej";
            header("Location: firma.php");
            exit();
        }
    }
        ...
        <div class="box">
            <form method="post">
                <table>
                    ...
                    <tr>
                        <td></td>
                        <td><input type="button" value="Usun dział" onclick="location.href='usundzial.php?id=<?php echo $edytuj_id; ?>'" style="background-color: #dc524c;"></td>
                    </tr>
                </table>
            </form>
        </div>
