<?php
    ...
    if (isset($_GET['id'])) $edytuj_id = $_GET['id'];
        ...
        $_SESSION[blad] = "Nie można usunąć stanowiska. Spróbuj ponownie póżniej";
        header("Location: firma.php");
        exit();
    }
    
    //czy zawiera pracowników
    $zap = "SELECT * FROM pracownicy WHERE ID_stanowiska = ".$edytuj_id;
    $query = mysqli_query($pol, $zap);
    
    if (mysqli_num_rows($query)>0){
        $_SESSION[blad] = "Nie można usunąć stanowiska, gdyż są do niego przypisani pracownicy!!!";
        header("Location: firma.php");
        exit();
    }

    //usuwanie stanowiska
    $zap1 = "DELETE FROM stanowiska WHERE ID = ".$edytuj_id;
    if (mysqli_query($pol, $zap1)){
        $_SESSION['sukces'] = "Usunięto stanowisko";
        header("Location: firma.php");
        exit();
    }
    else{
        $_SESSION['blad'] = "Nie udało się usunąć stanowiska. Spróbuj ponownie póżniej";
        header("Location: firma.php");
        exit();
    }
